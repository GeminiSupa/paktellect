...
AS $$
BEGIN
  /** Requirements are now optional to allow faster onboarding. */
  RETURN NEW;
END;
$$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='teachers' AND column_name='category') THEN
        ALTER TABLE teachers ADD COLUMN category TEXT DEFAULT 'Academic' CHECK (category IN ('Academic', 'Legal', 'Wellness', 'Mental Health', 'Plumbing', 'Electrical', 'Logistics', 'Mechanics'));
    END IF;
